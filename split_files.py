# This script splits a large JSON file into smaller files, each one containing a specified number of lines.

input_file = "yelp_academic_dataset_review.json"
output_prefix = "split_file_"
num_files = 20

# Count total lines (objects) in the file
with open(input_file, "r" , encoding="utf8") as file:
    total_lines = sum(1 for _ in file)

lines_per_file = total_lines // num_files  # Lines per split file

print(f"Total lines: {total_lines}, Lines per file: {lines_per_file}")

# Split into multiple smaller files
with open(input_file, "r" , encoding="utf8") as file:
    for i in range(num_files):
        output_filename = f"{output_prefix}{i+1}.json"

        with open(output_filename, "w", encoding="utf8" ) as out_file:
            for j in range(lines_per_file):
                line = file.readline()
                if not line:
                    break  # Stop if file ends early
                out_file.write(line)

print("JSON file has been successfully split into smaller chunks")